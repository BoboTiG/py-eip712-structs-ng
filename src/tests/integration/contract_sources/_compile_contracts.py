...
arg_parser.add_argument("-v", "--version", help="Solidity version for compiling contracts.")
...
arg_parser.add_argument("-o", "--optimize", action="store_true", help="Optimize the contract")
user_args = arg_parser.parse_args()

CONFIGURED_SOLIDITY_VERSION = "0.8.19"
# establish Solidity version from user-provided arg or use hard-coded version
user_sol_version = user_args.version

solidity_version = user_sol_version or CONFIGURED_SOLIDITY_VERSION
...
def _compile_dot_sol_files(dot_sol_filename: str, /) -> dict[str, Any]:
    return solcx.compile_files(
        ...
        optimize=user_args.optimize,
        optimize_runs=200 if user_args.optimize else None,
    )
...
def compile_files(file_list: list[str], /) -> None:
    ...
    for dot_sol_filename in contracts_in_file.keys():
        ...
        split_and_lowercase = [i.lower() for i in re.split(r"([A-Z][a-z]*)", filename_no_extension) if i]
        ...
        with open(python_file_path, "w") as f:
            ...
            for c in contracts_in_file[dot_sol_filename]:
                c_name_split_and_uppercase = [i.upper() for i in re.split(r"([A-Z0-9][a-z0-9]*)", c) if i]
                contract_upper = "_".join(c_name_split_and_uppercase)

                c_data = _get_compiled_contract_data(compiled_dot_sol_data, dot_sol_filename, c)

                contract_source = f"# source: ../{dot_sol_filename}:{c}"
